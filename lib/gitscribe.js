  ...
  */
  async generateBurndownChart(milestone, milestoneIssues, options = {}) {
    ...
    for (const issue of milestoneIssues) {
      ...
      const dayAdded = Math.max(0, Math.floor((new Date(lastMilestoneChangeComment.notes[0].updatedAt) - startDate) / MILLISECONDS_IN_DAY));

      // The day of the sprint where the issue was closed. Only valid if issue.closedAt is not null
      const dayClosed = issue.closedAt ? Math.floor((new Date(issue.closedAt) - startDate) / MILLISECONDS_IN_DAY) : null;
